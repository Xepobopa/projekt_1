...
public class BroadcastTask implements Runnable {
	...
	public void run() {
		...
		try (DatagramSocket dataSocket = new DatagramSocket(Consts.FILE_SERVER_PORT)) {
			while (true) { 
				...
				// ulozim si, lebo mozno pouzijem pri poslednom chunku a mozno ne
				...
				oos.write(chunkFromFile, 0, reallyRead); // because last chunk can be smaller than DATA_CHUNK_SIZE
				oos.flush();
				
				DatagramPacket requestData = new DatagramPacket(baos.toByteArray(), baos.toByteArray().length,
				InetAddress.getByName("localhost"), Consts.DATA_CLIENT_PORT);
				dataSocket.send(requestData);
			}
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
